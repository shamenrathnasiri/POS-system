...
export default function SalesHistory() {
  ...
  const fetchSales = useCallback(async () => {
    try {
      setIsLoading(true);
      const res = (await salesApi.getAll(page, 20)) as any;
  ...
  const viewDetails = async (sale: Sale) => {
    try {
      const res = (await salesApi.getById(sale.id)) as any;
          ...
          {selectedSale && (
            <div className="space-y-4">
              ...
              {selectedSale.items?.map((item, idx) => (
                <div key={idx} className="flex justify-between text-sm p-2 rounded-lg bg-white/[0.02]">
