...
import { ApiResponse, DailyReport, MonthlyReport, Product, TopProduct, LowStockReport } from "@/types";
...
export default function ReportsPage() {
  ...
  const fetchDaily = useCallback(async () => {
    try {
      const response = (await reportsApi.daily(dailyDate)) as ApiResponse<DailyReport>;
  ...
  const fetchMonthly = useCallback(async () => {
    try {
      const response = (await reportsApi.monthly(`${monthlyDate}-01`)) as ApiResponse<MonthlyReport>;
  ...
  const fetchTop = useCallback(async () => {
    try {
      const response = (await reportsApi.topProducts(10)) as ApiResponse<{ topProducts: TopProduct[] }>;
  ...
  const fetchLow = useCallback(async () => {
    try {
      const response = (await reportsApi.lowStock()) as ApiResponse<LowStockReport>;
    ...
    <ScrollArea className="h-full">
      <div className="p-6 space-y-6">
        ...
        <Tabs defaultValue="daily" className="space-y-4">
          <TabsList className="bg-white/5 border border-white/6">
            ...
            {dailyData && (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {[
                  ...
                  <Card key={i} className="border-white/6 bg-white/2">
                ...
                <Card className="border-white/6 bg-white/2">
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      {monthlyData.dailyBreakdown?.map((d, i) => (
                        <div key={i} className="flex items-center gap-3">
                          <span className="text-xs text-white/40 w-24">{d.date}</span>
                          <div className="flex-1 h-6 bg-white/5 rounded-full overflow-hidden">
                            <div className="h-full bg-linear-to-r from-blue-500 to-indigo-500 rounded-full transition-all"
                              style={{ width: `${Math.min(100, (Number(d.revenue) / Math.max(...monthlyData.dailyBreakdown.map((x) => Number(x.revenue) || 1))) * 100)}%` }} />
                          </div>
                          <span className="text-xs text-white/60 w-28 text-right">Rs. {Number(d.revenue).toFixed(2)}</span>
              ...
              <CardContent>
                {topProducts.length === 0 ? <p className="text-white/30 text-center py-8">No data</p> : (
                  <div className="space-y-3">
                    {topProducts.map((p, i) => (
                      <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-white/2 border border-white/4">
                        ...
                        <p className="text-sm font-semibold text-emerald-400">Rs. {Number(p.total_revenue).toFixed(2)}</p>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <Card className="border-white/6 bg-white/2">
              ...
              <CardContent>
                {lowStock.length === 0 ? <p className="text-white/30 text-center py-8">All stocked ✓</p> : (
                  <div className="space-y-3">
                    {lowStock.map((p) => (
                      <div key={p.id} className="flex items-center gap-3 p-3 rounded-lg bg-white/2 border border-white/4">
